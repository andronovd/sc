...
//used for finding the appropriate threshold for Hough
#define MAX_T 200
#define MIN_T 0

#define PRINT 0 //show the print debug statements
#define SHOW 0 //Show the images along the ways
...
void findSpots( char* filename, vector<spot*>* spots, int galCount)
{
	src = imread( filename );
...
void Hough( vector< Vec3f >* circles, int galCount, Mat* tmp )
{
	int min = MIN_T;
	int max = MAX_T;
	int currThresh = MAX_T;
	printf( "galCount: %d\n", galCount );
	
	while( galCount != circles->size() )
	{
		HoughCircles( *tmp, *circles, CV_HOUGH_GRADIENT, 1, tmp->rows/8, thresh, thresh, 0, 0 );
		if( galCount < circles->size() )
		{
			max = currThresh;
			currThresh = max - ( ( max - min ) >> 1 );
		}
		else if( galCount > circles->size() )
		{
			min = currThresh;
			currThresh = min + ( (max - min ) >> 1 );
		}
		printf( "max: %d, min: %d, currThresh: %d, No.Circles: %d\n", max, min, currThresh, circles->size() );
		circles->clear();
		if( currThresh == max || currThresh == min )
		{
			printf( "Unable to find all printed dots\n" );
			break;
		}
